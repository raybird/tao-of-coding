...
Path override options:
  --root-skill-path <path>      Default: skills/tao-of-opencode/SKILL.md
  --role-guide-path <path>      Override role guide path
  --skill-file <path>           Override target skill file path
  --skill-root <path>           Default: skills/tao-of-opencode/references/superpowers
...
SKILL_STACK=""
VISITED_SKILLS=""
ROOT_SKILL_PATH="skills/tao-of-opencode/SKILL.md"
ROLE_GUIDE_PATH=""
SKILL_FILE=""
SKILL_ROOT="skills/tao-of-opencode/references/superpowers"
OUTPUT_FILE=""
RUNNER_CMD=""
DRY_RUN=0
...
if [[ "$EXECUTION_MODE" == "delegated" ]]; then
  if [[ "$FORBID_ROOT_RELOAD" == "true" && "$SKILL" == "tao-of-opencode" ]]; then
    fail "E_ROOT_RELOAD_BLOCKED" "delegated mode cannot target tao-of-opencode root"
  fi
...
if [[ -z "$ROLE_GUIDE_PATH" ]]; then
  ROLE_GUIDE_PATH="skills/tao-of-opencode/references/${ROLE}.md"
fi
if [[ -z "$SKILL_FILE" ]]; then
  if [[ "$SKILL" == "tao-of-opencode" ]]; then
